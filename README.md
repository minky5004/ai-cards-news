...
- **배포 호출** — `GITHUB_TOKEN` 푸시는 워크플로를 발화시키지 못한다(재귀 방지). 부를 때 발행 커밋의 SHA 를 넘겨, 그 사이 다른 푸시가 들어와도 그날 카드가 그대로 나간다
...
- 예약이 1~2시간 늦게 발화한다(5일 관측 · 1시간 26분~2시간 15분) — 그날 안에만 나가면 되므로 방치
