...
public final class Gemini {
    ...
    /**
     * 아이디어 모델이 막혔을 때 카피 모델로 한 번 더 던지는 요청의 상한.
     *
     * <p>1 인 이유는 <b>카피의 분당 창</b>이다. 아이디어는 카피 직후에 돌고 자기 재시도 대기의 합이
     * 60초 미만이라, 폴백 요청은 카피 마지막 두 기사의 창에 떨어질 수 있다. 그 창의 카피 최악이
     * 2기사 x {@link #COPY_MAX_ATTEMPTS} = 4 라 RPM 5 에 남는 자리가 하나다.
     */
     ...
     * 이었고 같은 날 3.6 은 카피 5/5 였다 — 반나절짜리 과부하 앞에서 바꿀 수 있는 것은 모델뿐이다.
     * 넘어가는 곳이 카피 모델인 이유는 <b>그날 카드를 낸 모델</b>이라서다. 아이디어 단계는 카피가
     * 끝난 뒤에 돈다.
     *
     * <p>넘어가는 것은 아이디어 모델 쪽 사정으로 막힌 실패뿐이다 — 재시도 대상 코드와 429(장부가
     * 모델별이라 아이디어 모델의 장부가 찬 것은 카피 모델과 무관하다). 파싱 실패(0)와 400 은 모델을
     * 바꿔도 같다.
     */
    public static Optional<String> ideaFallback(String ideaModel, String copyModel, int status) {
        boolean blocked = RETRY_STATUS_CODES.contains(status) || status == TOO_MANY_REQUESTS;
        return blocked && !ideaModel.equals(copyModel) ? Optional.of(copyModel) : Optional.empty();
    }
